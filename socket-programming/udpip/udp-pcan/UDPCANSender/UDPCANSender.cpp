...
UDPCANSender::UDPCANSender()
{
	...
	m_ThreadRun = true;
	m_ReadThread = new std::thread(&UDPCANSender::ThreadExecute, this);
...
void UDPCANSender::ThreadExecute()
{
	while (m_ThreadRun)
	{
...
void UDPCANSender::ReadMessages()
{
	TPCANStatus stsResult;
...
TPCANStatus UDPCANSender::ReadMessageFD()
{
	TPCANMsgFD CANMsg;
...
TPCANStatus UDPCANSender::ReadMessage()
{
	TPCANMsg CANMsg;
...
void UDPCANSender::ValidateId(TPCANMsg msg)
{
...
void UDPCANSender::ProcessMessageCan(TPCANMsg msg, TPCANTimestamp itsTimeStamp)
{
...
void UDPCANSender::ProcessMessageCanFD(TPCANMsgFD msg, TPCANTimestampFD itsTimeStamp)
{
	std::cout << "Type: " << GetMsgTypeString(msg.MSGTYPE) << std::endl;
	std::cout << "ID: " << GetIdString(msg.ID, msg.MSGTYPE) << std::endl;
...
void UDPCANSender::ShowConfigurationHelp()
{
	std::cout << "=========================================================================================" << std::endl;
	std::cout << "|                           PCAN-Basic UDPCANSender Example                                |" << std::endl;
	std::cout << "=========================================================================================" << std::endl;
...
void UDPCANSender::ShowCurrentConfiguration()
{
...
void UDPCANSender::ShowStatus(TPCANStatus status)
{
	std::cout << "=========================================================================================" << std::endl;
	char buffer[MAX_PATH];
...
void UDPCANSender::FormatChannelName(TPCANHandle handle, LPSTR buffer, bool isFD)
{
...
void UDPCANSender::GetTPCANHandleName(TPCANHandle handle, LPSTR buffer)
{
...
void UDPCANSender::GetFormattedError(TPCANStatus error, LPSTR buffer)
{
...
void UDPCANSender::ConvertBitrateToString(TPCANBaudrate bitrate, LPSTR buffer)
{
	switch (bitrate)
	{
	case PCAN_BAUD_1M:
...
std::string UDPCANSender::GetMsgTypeString(TPCANMessageType msgType)
{
...
std::string UDPCANSender::GetIdString(UINT32 id, TPCANMessageType msgType)
{
	char result[MAX_PATH] = { 0 };
	if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
	{
...
int UDPCANSender::GetLengthFromDLC(BYTE dlc)
{
	switch (dlc)
	{
	case 9: return 12;
...
std::string UDPCANSender::GetTimeString(TPCANTimestampFD time)
{
	char result[MAX_PATH] = { 0 };
...
std::string UDPCANSender::GetDataString(BYTE data[], TPCANMessageType msgType, int dataLength)
{
	if ((msgType & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR)
...
void UDPCANSender::Send(TPCANMsg msg)
{
	/// Send data to client
	/// buffer = Data + ID
	...
	retVal = sendto(m_socket, buffer, bufferSize, 0, (SOCKADDR*)&senderAddr, sizeof(senderAddr));
	if (retVal == SOCKET_ERROR)
	{
